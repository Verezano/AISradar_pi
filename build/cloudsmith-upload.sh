...
PROD_REPO=${CLOUDSMITH_PROD_REPO:-'opencpn/aisradar-prod'}
BETA_REPO=${CLOUDSMITH_BETA_REPO:-'opencpn/aisradar-beta'}
ALPHA_REPO=${CLOUDSMITH_ALPHA_REPO:-'opencpn/aisradar-alpha'}
...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=aisradar_pi-1.2.6.0-${PKG_TARGET}-x86_64-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=aisradar_pi-1.2.6.0-${PKG_TARGET}-x86_64-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="1.2.6.0+${BUILD_ID}.${commit}"
...
if [ $LOCAL_BUILD = false ]; then
    cloudsmith push raw --republish --no-wait-for-sync \
        --name aisradar_pi-1.2.6.0-ubuntu-x86_64-20.04-${OCPN_TARGET}-metadata \
        --version ${VERSION} \
        --summary "aisradar opencpn plugin metadata for automatic installation" \
        $REPO $xml

    cloudsmith push raw --republish --no-wait-for-sync \
        --name $tarball_name \
        --version ${VERSION} \
        --summary "aisradar opencpn plugin tarball for automatic installation" \
        $REPO $tarball

    if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name opencpn-package-aisradar-1.2.6.0-ubuntu-x86_64-20.04-${OCPN_TARGET}.${PKG_EXT} \
            --version ${VERSION} \
            --summary "aisradar .${PKG_EXT} installation package" \
            $REPO $pkg
    fi
fi
